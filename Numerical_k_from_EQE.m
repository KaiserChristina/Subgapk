...
%=================user input=========================
% Choose a project directory which contains the experimental data as .xls
% files and to which figures will be saved.
projectdir = 'C:\Users\Christina\Dropbox (Ser Sam)\Christina Data and Literature\Numerical k Paper\Numerical_k_from_EQE';
...
% %==================================Export figures to projectdir
% h =  findobj('type','figure');
% figure_name = flip({h.Name});
% n_Fig = length({h.Number});
%  
% for figure_index = 1:n_Fig
% baseFileName = sprintf(figure_name{figure_index}); 
% fullFileName = fullfile(projectdir, baseFileName(1:end)); 
% figure(figure_index); % Activate the figure again for print
% print(fullFileName,'-dtiff','-r500');
% end
